...
describe('<YearInput />: parseInput', () => {
  describe('`value` prop provided', () => {
    it('create moment from input string', () => {
      const wrapper = shallow(<YearInput
        value="2018-08-19" />);
  
      assert(moment.isMoment(wrapper.instance().parseInput()), 'return moment instance');
      assert(wrapper.instance().parseInput().isValid(), 'return valid moment instance');
      assert(wrapper.instance().parseInput().isSame(moment('2018-08-19'), 'year'), 'return correct moment');
    });
  });

  describe('`value` prop is not provided', () => {
    it('return undefined', () => {
      const wrapper = shallow(<YearInput />);
  
      assert(_.isUndefined(wrapper.instance().parseInput()), 'return undefined');
    });
  });
});
